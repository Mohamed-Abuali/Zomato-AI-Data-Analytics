...
with DAG(
    ...
    dbt_build_core = BashOperator(
        ...
        bash_command=f"{DBT} build exclude tag:ai --project-dir {DBT_PROJECT} --profile-dir {DBT_PROJECT}",
    )
    ...
    dbt_build_ai = BashOperator(
        ...
        bash_command=f"{DBT} select tag:ai --project-dir {DBT_PROJECT} --profile-dir {DBT_PROJECT}",
    )
